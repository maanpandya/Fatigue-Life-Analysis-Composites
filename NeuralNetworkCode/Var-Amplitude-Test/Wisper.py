...
# Read the file
with open('NeuralNetworkCode\Var-Amplitude-Test\WISPERX', 'r') as file:
...
cyclesSN, stressesSN = SNfunction(normalized_array, list(range(200, 370)), surface)
cyclesNN, stressesNN = NNfuntion(normalized_array, list(range(200, 370)))
...
print("stressesNN")
print(cyclesNN)
plt.scatter(np.log(anotherone), np.log(j), color = "black", label = "CLD")
plt.scatter(np.log(anotherone), np.log(j), color = "blue", label = "PINN")
